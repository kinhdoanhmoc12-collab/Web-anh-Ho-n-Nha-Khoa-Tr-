...
import { ArrowRight, ChevronRight, Download, Eye } from "lucide-react";
    ...
    <div className="space-y-4 mb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {itemsList.map((item) => (
          <div
            key={item.id}
            className="group relative rounded-xl overflow-hidden shadow-md bg-slate-900 border border-slate-200/40 hover:shadow-xl transition-all duration-300"
          >
            ...
            <div className="relative aspect-[4/3] overflow-hidden">
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                <button className="p-2.5 rounded-full bg-[#d9534f] text-white hover:scale-110 transition-transform">
                  <Download className="w-4 h-4" />
                </button>
                <button className="p-2.5 rounded-full bg-slate-800 text-white hover:scale-110 transition-transform">
